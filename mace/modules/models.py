...
print("Using recurrent architecture for MACE, BOTH interaction and product blocks are shared weights")
@compile_mode("script")
class MACE(torch.nn.Module):
    def __init__(
        ...
        # Shared weights layers (middle layers) (Layer 2 to num_interactions - 2)
        if num_interactions > 3:
            self.shared_inter = interaction_cls(
                node_attrs_irreps=node_attr_irreps,
                node_feats_irreps=hidden_irreps,
                edge_attrs_irreps=sh_irreps,
                edge_feats_irreps=edge_feats_irreps,
            ...
            self.shared_prod = EquivariantProductBasisBlock(
                node_feats_irreps=interaction_irreps,
                target_irreps=hidden_irreps,
                correlation=correlation[2],  # note: still using correct correlation[i]
                num_elements=num_elements,
                use_sc=True,
                cueq_config=cueq_config,
            )
            ...
            for i in range(2, num_interactions - 1):
                self.interactions.append(self.shared_inter)
                self.products.append(self.shared_prod)
                self.readouts.append(
                    LinearReadoutBlock(hidden_irreps, o3.Irreps(f"{len(heads)}x0e"), cueq_config)
                )
...
# # Add recurrent architecture
# print("Using recurrent architecture for MACE, only interaction block are shared weights")
# @compile_mode("script")
# class MACE(torch.nn.Module):
#     def __init__(
#         self,
