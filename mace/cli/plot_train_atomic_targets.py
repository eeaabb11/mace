...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot MACE training statistics for atomic targets")
    parser.add_argument(
...
def plot(data: pd.DataFrame, min_epoch: int, output_path: str) -> None:
    ...
    # Plot MAE Atomic Target & RMSE Atomic Target
    ax = axes[1]
    if "mae_atomic_target" in valid_data:
        ax.plot(
            valid_data["epoch"],
            valid_data["mae_atomic_target"]["mean"],
            color=colors[1],
            zorder=1,
            label="MAE Atomic Target",
        )
        ax.fill_between(
            x=valid_data["epoch"],
            y1=valid_data["mae_atomic_target"]["mean"] - valid_data["mae_atomic_target"]["std"],
            y2=valid_data["mae_atomic_target"]["mean"] + valid_data["mae_atomic_target"]["std"],
            alpha=0.5,
            zorder=-1,
            color=colors[1],
        )

    if "rmse_atomic_target" in valid_data:
        ax.plot(
            valid_data["epoch"],
            valid_data["rmse_atomic_target"]["mean"],
            color=colors[2],
            zorder=1,
            label="RMSE Atomic Target",
        )
        ax.fill_between(
            x=valid_data["epoch"],
            y1=valid_data["rmse_atomic_target"]["mean"] - valid_data["rmse_atomic_target"]["std"],
            y2=valid_data["rmse_atomic_target"]["mean"] + valid_data["rmse_atomic_target"]["std"],
            alpha=0.5,
            zorder=-1,
            color=colors[2],
        )
