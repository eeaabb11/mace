...
###########################################################################################
print("+++++atomic mask verison, model saves based on last epoch+++")
import argparse
import ast
import glob
import json
...
def run(args: argparse.Namespace) -> None:
    ...
    # Define a dummy checkpoint handler to prevent AttributeError
    ...
    # Modify the train function call to ensure final model saving
    checkpoint_handler = DummyCheckpointHandler()  # Use the dummy checkpoint handler
    ...
    tools.train(
        ...
        checkpoint_handler=checkpoint_handler,  # Use the dummy object
        eval_interval=args.eval_interval,
        start_epoch=start_epoch,
        max_num_epochs=args.max_num_epochs,
        logger=logger,
        patience=args.patience,
        save_all_checkpoints=False,  # No intermediate checkpoints
    ...
    if rank == 0:
        model_path = Path(args.checkpoints_dir) / (tag + "_last_epoch.model")
        logging.info(f"Saving last epoch model to {model_path}")
        if args.save_cpu:
            model = model.to("cpu")
    ...
    # last_epoch_model_path = Path(args.checkpoints_dir) / (tag + "_last_epoch.model")
    # torch.save(model, last_epoch_model_path)
